...
const chatting = (io) => {
  io.on('connection', (socket) => {
    ...
    socket.on('remove-currentChat', async () => {
      await User.updateOne(
        { socketId: socket.id },
        { $set: { currentChat: '62b0b96c136f8a85dddddddd' } }
      );
    });
      ...
      try {
        ...
        if (receiverSocketId) {
          ...
          if (
            ...
            console.log(receiver.currentChat.toString());
            console.log(sender.currentChat.toString());
            message.seen = true;
            await Message.updateOne(
              { _id: message.id },
              { $set: { seen: true } }
